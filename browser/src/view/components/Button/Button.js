...
export const Button = ({ label, style, icon }) => {
  const renderButtonStyle = buttonStyle => {
    switch (buttonStyle) {
      case 'icon-only':
        return icon;
      case 'text-only':
  ...
  return <button className='btn-icon-container'>{renderButtonStyle(style)}</button>;
};

/*
Button:
- Style: icon only, text only, icon + text // type: normal, secondary, destructive
